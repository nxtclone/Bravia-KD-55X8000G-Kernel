...
#ifdef CC_AOS_PMIC
				mode_data |= (1 << 7); //change setting from External Mode to Internal Mode 
#endif
				break;
...
#ifdef CC_AOS_PMIC
static int mt53xx_regulator_aos_init(void)
{

	int i4RetVal;
	u8 mode_data;

		mode_data = _I2C_Xfer_Read_Cmd((u8) I2C_VCPU_DEVADDR, I2C_VCPU_MODADDR);
		mode_data &= ~I2C_VCPU_MODMASK;
		mode_data = (I2C_VCPU_MODPWM_EN == 1) ? (mode_data | I2C_VCPU_MODMASK) : mode_data;
		mode_data |= (1 << 7); //change setting from External Mode to Internal Mode 
		i4RetVal = _I2C_Xfer_Write_Cmd((u8) I2C_VCPU_DEVADDR, I2C_VCPU_MODADDR, mode_data);

		mode_data = _I2C_Xfer_Read_Cmd((u8) I2C_VGPU_DEVADDR, I2C_VGPU_MODADDR);
		mode_data &= ~I2C_VGPU_MODMASK;
		mode_data = (I2C_VGPU_MODPWM_EN == 1) ? (mode_data | I2C_VGPU_MODMASK) : mode_data;
		mode_data |= (1 << 7); //change setting from External Mode to Internal Mode 
		i4RetVal = _I2C_Xfer_Write_Cmd((u8) I2C_VGPU_DEVADDR, I2C_VGPU_MODADDR, mode_data);
