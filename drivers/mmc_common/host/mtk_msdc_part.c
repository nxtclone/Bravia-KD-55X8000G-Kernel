...
int msdc_partread(int partno, uint64_t offset, uint64_t size, void *buf)
{
	...
	if (!card) {
		pr_err("[%s]eMMC isn't init done before %s.\n",
				__func__, current->comm);
		// TODO: judge clk is on/off.
		BUG_ON(!card);
	}

	/* Firstly get the host */
	mmc_get_card(card);
	if (mmc_card_suspended(host->mmc->card)) {
		pr_err("mmc suspended, %s %s: partno=%d, " 
					"offset=0x%llx, size=0x%llx.\n",
					current->comm, __func__, partno, offset, size);
		/* release the host */
