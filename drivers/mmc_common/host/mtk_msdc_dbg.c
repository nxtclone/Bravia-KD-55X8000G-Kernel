...
static int msdc_debug_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "-> echo 0x17 1 1 >msdc_bebug -> host[1] drv mode DMA. (dma when >= 512)\n");
...
static ssize_t msdc_debug_write(struct file *file, const char *buf, size_t count, loff_t *f_pos)
{
	int ret;
...
static ssize_t msdc_tune_write(struct file *file, const char *buf, size_t count, loff_t *f_pos)
{
	int ret;
	int cmd, p1, p2;
