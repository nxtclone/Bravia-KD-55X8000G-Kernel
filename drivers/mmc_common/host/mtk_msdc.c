...
/*get the PCB module info.*/
extern unsigned int product_model;
...
#if defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886) || defined(CONFIG_ARCH_MT5893) || defined(CONFIG_ARCH_MT5895)
...
void msdc_set_driving(struct msdc_host *host, struct msdc_hw *hw)
{
	 ...
	 */
#if defined(CONFIG_ARCH_MT5893) || defined(CONFIG_ARCH_MT5895)
    ...
    if (host->hw->host_function == MSDC_EMMC) {
         ...
         * PD [24]{0xf000d51c, 0x1000000, 0x0000000},
...
void msdc_pinmux(struct msdc_host *host)
{
	...
	if (host->hw->host_function == MSDC_EMMC || host->hw->host_function == MSDC_SD) {
		...
		if(IS_IC_MT5883_ES1())
		{
			reg_base = ioremap(host->u4IO_Base + 0x78104, 4);
...
#if defined(CONFIG_EMMC_AES_SUPPORT) && defined(CC_MTD_ENCRYPT_SUPPORT)
...
static void msdc_config_hclk(struct msdc_host *host)
{
	...
	/* DMUX PLL default is 189MHZ for emmc, now down to 177MHZ for specific PCB*/
	if ((product_model != 0xFFFFFFFF) &&
		(product_model != 0x8) &&
		(product_model != 0xA)
		) {

		u32 u4old = 0;
		sdr_set_field(reg, (0x7f << 13), 0x3B); //0x3F = 189MHZ, 0x3B = 177MHZ
		sdr_get_field(reg, (0x1 << 20), u4old); //toggle the bit 20 to valid the clk change
		sdr_set_field(reg, (0x1 << 20), u4old ^ 1);
	}
...
static void msdc_apply_kernel_autok(struct msdc_host *host)
{
	...
	//dump the registers
	ERR_MSG("IOCON                = 0x%08x \n", sdr_read32(MSDC_IOCON));
	ERR_MSG("MSDC_PAD_TUNE0       = 0x%08x \n", sdr_read32(MSDC_PAD_TUNE0));
	ERR_MSG("MSDC_PAD_TUNE1       = 0x%08x \n", sdr_read32(MSDC_PAD_TUNE1));
	ERR_MSG("MSDC_PATCH_BIT0      = 0x%08x \n", sdr_read32(MSDC_PATCH_BIT0));
	ERR_MSG("MSDC_PATCH_BIT1      = 0x%08x \n", sdr_read32(MSDC_PATCH_BIT1));
	ERR_MSG("MSDC_PATCH_BIT2      = 0x%08x \n", sdr_read32(MSDC_PATCH_BIT2));
	ERR_MSG("EMMC50_CFG0          = 0x%08x \n", sdr_read32(EMMC50_CFG0));
	ERR_MSG("MSDC_DAT_RDDLY0      = 0x%08x \n", sdr_read32(MSDC_DAT_RDDLY0));
	ERR_MSG("MSDC_DAT_RDDLY1      = 0x%08x \n", sdr_read32(MSDC_DAT_RDDLY1));
	ERR_MSG("MSDC_DAT_RDDLY2      = 0x%08x \n", sdr_read32(MSDC_DAT_RDDLY2));
	ERR_MSG("MSDC_DAT_RDDLY3      = 0x%08x \n", sdr_read32(MSDC_DAT_RDDLY3));
	ERR_MSG("EMMC50_PAD_DS_TUNE   = 0x%08x , adjust delay %d -> %d \n", sdr_read32(EMMC50_PAD_DS_TUNE), old_ds_dly1, new_ds_dly1);
	ERR_MSG("EMMC50_PAD_CMD_TUNE  = 0x%08x \n", sdr_read32(EMMC50_PAD_CMD_TUNE));
	ERR_MSG("EMMC50_PAD_DAT01_TUNE= 0x%08x \n", sdr_read32(EMMC50_PAD_DAT01_TUNE));
	ERR_MSG("EMMC50_PAD_DAT23_TUNE= 0x%08x \n", sdr_read32(EMMC50_PAD_DAT23_TUNE));
	ERR_MSG("EMMC50_PAD_DAT45_TUNE= 0x%08x \n", sdr_read32(EMMC50_PAD_DAT45_TUNE));
	ERR_MSG("EMMC50_PAD_DAT67_TUNE= 0x%08x \n", sdr_read32(EMMC50_PAD_DAT67_TUNE));
}
...
static void msdc_set_mclk(struct msdc_host *host, msdc_state state, u32 hz)
{
	...
	if (state == MSDC_STATE_HS400 || state == MSDC_STATE_HS200) {
		...
		/*Specific PCB use larger driving*/
		if ((0xFFFFFFFF == product_model) ||
			(0x8 == product_model)  || (0xA == product_model) ||
			(0xB == product_model)  || (0xC == product_model) ||
			(0x11 == product_model) || (0x12 == product_model))
		{
		...
		} else if (( 0xE == product_model) || ( 0xF == product_model) || (0x10 == product_model)) {
			host->hw->clk_drv = 7;
			host->hw->cmd_drv = 7;
			host->hw->dat_drv = 7;
		}
		...
		host->hw->cmd_drv = 5;
		host->hw->dat_drv = 4;
    #elif defined(CONFIG_ARCH_MT5893) || defined(CONFIG_ARCH_MT5895)
		host->hw->clk_drv = 7;
		host->hw->cmd_drv = 7;
		host->hw->dat_drv = 5;
	#endif
	...
	pr_err("###set driving clk %d, cmd %d, dat %d, [product_model 0x%x]\n",
			host->hw->clk_drv, host->hw->cmd_drv, host->hw->dat_drv, product_model);
...
static int msdc_dma_config(struct msdc_host *host, struct msdc_dma *dma)
{
	...
	defined(CONFIG_ARCH_MT5863) || defined(CONFIG_ARCH_MT5866) || defined(CONFIG_ARCH_MT5893) || defined(CONFIG_ARCH_MT5895)
...
static int msdc_do_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (!data) {
		...
		#ifdef CC_MTD_ENCRYPT_SUPPORT
		spin_unlock(&host->lock);
		emmc_gcpu_aes_encrypt(host, cmd, data);
		spin_lock(&host->lock);
		#endif
	...
	if (data != NULL) {
		...
		if (dma != 0) {
			...
			if (map_sg == 1) {
				...
				#ifdef CC_MTD_ENCRYPT_SUPPORT
				spin_unlock(&host->lock);
                emmc_gcpu_aes_decrypt(host, cmd, data);
				spin_lock(&host->lock);
				#endif
			}
		}
...
static void msdc_post_req(struct mmc_host *mmc, struct mmc_request *mrq, int err)
{
	...
	if(data && data->host_cookie){
        ...
        //check decrypt
		#ifdef CC_MTD_ENCRYPT_SUPPORT
		emmc_gcpu_aes_decrypt(host, mrq->cmd, data);
		#endif
		data->host_cookie = 0;
...
{
    pr_err("Enter %s ...timing %d..mclk %d..\n",
			__func__, host->mmc->ios.timing, host->mclk);

	if (host->hw->host_function == MSDC_EMMC) {
		if (((host->mclk <= 52000000)) ||
			(!(g_autok_cmdline_parsed & MSDC_AUTOK_EXEC_DONE))) {
			/*
			 * before we really change to HS200/HS400,
			 * we still use HS mode tuning.
			 */
...
static void msdc_ops_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	ERR_MSG("set ios, power_mode=%d, bus_width=%d, clock = %d, timing = %d\n",
		ios->power_mode, ios->bus_width, ios->clock, ios->timing);
	...
	if (host->mclk != ios->clock || host->state != state) { /* not change when clock Freq. not changed ddr need set clock*/
		...
		if (ios->clock == 0) {
			...
			sdr_write32(MSDC_PAD_TUNE0,   0x00000000);
			sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_CMD_RSP_TA_CNTR, 0);
			sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_WRDAT_CRCS_TA_CNTR, 0);
		...
		#if defined(CONFIG_ARCH_MT5893) || defined(CONFIG_ARCH_MT5887) || defined(CONFIG_ARCH_MT5895)
...
#endif
#if defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886) || defined(CONFIG_ARCH_MT5863) || defined(CONFIG_ARCH_MT5866) || defined(CONFIG_ARCH_MT5893) || defined(CONFIG_ARCH_MT5895)
	if (mmc->ios.timing == MMC_TIMING_UHS_DDR50)
...
static void msdc_init_hw(struct msdc_host *host)
{
	...
	sdr_set_field(EMMC50_SD, (0x1 << 0), 1);
	...
	if(host->hw->host_function == MSDC_SD)
		...
		sdr_write32(MSDC_PATCH_BIT1, 0xFFFE0000);
		#else
	...
	#if defined(CONFIG_ARCH_MT5893)
	sdr_set_field(MSDC_PATCH_BIT0, MSDC_PB0_DESCUP, 1);
	sdr_set_field(MSDC_PATCH_BIT0, MSDC_PB0_RESV1, 1);
	sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_BIAS_TUNE_28NM, 6);
	sdr_set_field(MSDC_PATCH_BIT2, MSDC_PB2_RESPWAITCNT, 3);
	sdr_set_field(MSDC_PATCH_BIT2, MSDC_PB2_POPENCNT, 0);
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev, "kernel 4.4  driver probe!\n");
    ...
    pr_err("~~msdc DTS host reg base 0x%x ~~\n",u4host_reg_base);
#else
    host->u4IO_Base = IO_VIRT;
#endif
    pr_err("~~msdc IO address base 0x%x ~~\n",host->u4IO_Base);
	msdc_config_mmc(mmc);
...
static const struct dev_pm_ops mtk_msdc_pm_ops = {
	SET_LATE_SYSTEM_SLEEP_PM_OPS(msdc_drv_suspend, msdc_drv_resume)
};
