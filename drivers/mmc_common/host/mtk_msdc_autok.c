...
#define AUTOK_CMD_LATCH_EN_HS400_VALUE          (0)
#define AUTOK_CRC_LATCH_EN_HS400_VALUE          (0)
#define AUTOK_RESPWAITCNT                       (3)
#define AUTOK_CRC_MA_VALUE                      (0)
...
static int autok_init_hs400(struct msdc_host *host)
{
    ...
    #if defined(CONFIG_ARCH_MT5887) || defined(CONFIG_ARCH_MT5893) || defined(CONFIG_ARCH_MT5895)
	sdr_set_field(SDC_CMD_STS, SDC_CMD_RX_ENHANCE_EN, 1);
	sdr_set_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CLKTXDLY, 3);
	sdr_set_field(MSDC_PATCH_BIT2, MSDC_PB2_RESPWAITCNT, AUTOK_RESPWAITCNT);
...
*/
	...
	if (uTuneSepFlag == 0) {
		...
		autok_tuning_parameter_init(host, p_autok_tune_res);
		pr_err("[AUTOK]Step2.Scan Per-bit DATA Pad Delay\r\n");
		for (i = 0; i < 8; i++) {
		...
		/* set DLY3 default value 20. */
		p_autok_tune_res[EMMC50_DS_ZDLY_DLY] = 20;
		sdr_set_field(EMMC50_PAD_DS_TUNE, MSDC_EMMC50_PAD_DS_TUNE_DLY3, 20);
	}
#endif
//#endif HS400_DSCLK_NEED_TUNING

HS400_TUNING_END:

	autok_tuning_parameter_init(host, p_autok_tune_res);
	sdr_get_field(MSDC_PAD_TUNE0, MSDC_PAD_TUNE0_CLKTXDLY, u4ClkTxDly);
	pr_err("[AUTOK][write data TXdly : %08x ]r\n", u4ClkTxDly);
#if AUTOK_PARAM_DUMP_ENABLE
...
void msdc_kernel_fifo_path_sel_init(struct msdc_host *host)
{
	...
	#if defined(CONFIG_ARCH_MT5893)
	sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_CMD_RSP_TA_CNTR, 0); /*patch_bit1[3], TA*/
	sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_WRDAT_CRCS_TA_CNTR, 0);/*patch_bit1[0], TA*/
	#else
	...
	#if defined(CONFIG_ARCH_MT5893)
	sdr_set_field(MSDC_PATCH_BIT1, MSDC_PB1_BIAS_TUNE_28NM, 6);/*[8-11]*/
