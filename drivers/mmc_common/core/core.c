...
static void __mmc_start_request(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	/* Mark this for now, we need to study the retune algorithm */
#if 0
...
int __mmc_claim_host(struct mmc_host *host, atomic_t *abort)
{
	...
	while (1) {
		if ((host->claimer != NULL) && time_after(jiffies, timeout)) {
			printk("thread %s claimed host already, cnt is %d,claim is %d,"
...
void mmc_init_erase(struct mmc_card *card)
{
	...
	if (mmc_card_sd(card) && card->ssr.au) {
		...
		card->pref_erase = 0;
	pr_debug("card->pref_erase=0x%lx\n", card->pref_erase);
}
...
#endif
	pr_debug("!!!!!! SKIP card max_discard and use timeout to measure the max_discard number\n");
