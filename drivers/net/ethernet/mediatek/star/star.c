...
#if defined(CONFIG_ARCH_MT5882) || defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886)
...
static int star_ioctl(struct net_device *dev, struct ifreq *req, int cmd)
{
    ...
    switch (cmd)
    {
        ...
        {
		struct ioctl_wop_para_cmd32 __user *req_ifr_data32;
		struct ioctl_wop_para_cmd32 wop_cmd32;
		struct ioctl_wop_para_cmd __user *req_ifr_data;
...
#ifdef CONFIG_COMPAT
		compat_flag = (is_compat_task() != 0) ? true : false;

		if (compat_flag) {
			req_ifr_data32 = compat_ptr(ptr_to_compat(
					 req->ifr_data));

			if (copy_from_user(&wop_cmd32, req_ifr_data32,
			    sizeof(struct ioctl_wop_para_cmd32))) {
				STAR_MSG(STAR_ERR,"req_ifr_data32: 0x%p fail! \n",
					    req_ifr_data32);
				return -EFAULT;
			}

			port_array = kmalloc((sizeof(u32) *
				     wop_cmd32.port_count), GFP_KERNEL);
			if (!port_array) {
				STAR_MSG(STAR_ERR,"port_array kmalloc fail! \n");
				return -EFAULT;
			}

			wop_port_array = compat_ptr(wop_cmd32.port_array);

			retval = copy_from_user(port_array, wop_port_array,
				 (sizeof(u32) * wop_cmd32.port_count));
			if (retval) {
				STAR_MSG(STAR_ERR,"wop_port_array: 0x%p fail! "
					    "count32 = %d, ret = %d \n",
					    wop_port_array,
					    wop_cmd32.port_count, retval);
				return -EFAULT;
			}

			retval = star_setWOPProtocolPort(wop_cmd32.protocol_type,
                    port_array, wop_cmd32.port_count);
		} else
#endif
			{
			req_ifr_data = req->ifr_data;

			if (copy_from_user(&wop_cmd, req_ifr_data,
			    sizeof(struct ioctl_wop_para_cmd))) {
				STAR_MSG(STAR_ERR,"req_ifr_data: 0x%p fail! \n",
					    req_ifr_data);
				return -EFAULT;
			}

			port_array = kmalloc((sizeof(u32) * wop_cmd.port_count),
				     GFP_KERNEL);
			if (!port_array) {
				STAR_MSG(STAR_ERR,"port_array kmalloc fail! \n");
				return -EFAULT;
			...
			retval = copy_from_user(port_array, wop_port_array,
				 (sizeof(u32) * wop_cmd.port_count));
			if (retval) {
				STAR_MSG(STAR_ERR,"wop_port_array: 0x%p fail! "
					    "count = %d, ret = %d \n",
					    wop_port_array, wop_cmd.port_count,
					    retval);
				return -EFAULT;
			}

			retval = star_setWOPProtocolPort(wop_cmd.protocol_type,
...
#endif
#if  defined(CONFIG_OPM)||defined(CONFIG_ARCH_MT5881)||defined(CONFIG_ARCH_MT5399) || defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5882) || defined(CONFIG_ARCH_MT5883)|| defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886)
u32 fgOpenStBeforeSuspend=FALSE;//to record open status before enter suspend for recovery after resume
int star_suspend(struct platform_device *pdev, pm_message_t state)
{
#if defined(CONFIG_ARCH_MT5881) || defined(CONFIG_ARCH_MT5399) || defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5882) || defined(CONFIG_ARCH_MT5883)|| defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886)
    #if defined(ETH_SUPPORT_WOL)
    u32 u4Reg = 0;
    u8 i;
    #endif
    ...
    if(netdev!=NULL)
    {
        #if defined (CONFIG_ARCH_MT5398) || defined (CONFIG_ARCH_MT5399) || defined (CONFIG_ARCH_MT5880) || defined (CONFIG_ARCH_MT5881) || defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5882) || defined(CONFIG_ARCH_MT5883)|| defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886)
#if defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886)
		if (IS_IC_MT5891_ES1())
			star_stop(netdev);
		else
...
struct platform_device *mtk_star_platform_device;

void mtk_vdec_star_suspend(void)
{
	STAR_MSG(STAR_ERR, "call mtk_vdec_star_suspend\n");
	star_suspend(mtk_star_platform_device, PMSG_SUSPEND);
}
EXPORT_SYMBOL(mtk_vdec_star_suspend);
...
#if defined(CONFIG_ARCH_MT5882) || defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5886)
	StarClearBit(STAR_T8032_INTEN(pdwnc_base), T8032_ETHERNET_INTEN);
#endif
...
void mtk_vdec_star_resume(void)
{
	STAR_MSG(STAR_ERR, "call mtk_vdec_star_resume\n");
	star_resume(mtk_star_platform_device);
}
EXPORT_SYMBOL(mtk_vdec_star_resume);
...
static int star_probe(struct platform_device *pdev)
{
	...
	netdev->irq = irq_of_parse_and_map(np1, 0);
	if (netdev->irq < 0) {
