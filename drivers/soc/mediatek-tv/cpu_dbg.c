...
EXPORT_SYMBOL(unlock_cpu_dbgpcsr);
...
void show_cpu_dbgpcsr(void) {
    ...
    const int n_sample = 10;  /* To observe variations at hard lockup with CONFIG_BOOTPARAM_HARDLOCKUP_PANIC */
    int n;
    ...
    for (n = 0; n < n_sample; n++) {
        for_each_possible_cpu(i) {
            ...
            dbgpcsr = (unsigned long)dbgpcsrhi << 32 | dbgpcsrlo;	// dbgpcsrhi is dropped in 32bit environment.
            pr_err("CPU%d PC: 0x%x_%x	%pF\n", i, dbgpcsrhi, dbgpcsrlo, (void*)dbgpcsr);
        }
    }
    #endif
}
