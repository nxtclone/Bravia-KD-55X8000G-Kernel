...
{
    char **endptr = NULL;   /* local pointer to end of parsed string */
    mt53xx_fb_start = simple_strtoull(str, endptr, 16);
...
{
    char **endptr = NULL;   /* local pointer to end of parsed string */
    mt53xx_fb_size = simple_strtoull(str, endptr, 16);
...
{
	if (start) {
		*start = (unsigned long long)(_data);
	}

	if (end) {
		*end = (unsigned long long)(_end);
	}
...
static int __init mt53xx_init_machine(void)
{
	...
	arch_setup_dma_ops(&(mt53xx_mtal_device.dev), 0, 0x0, NULL, false);
#ifdef CONFIG_CMA
