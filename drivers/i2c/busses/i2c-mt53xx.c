...
struct mtk_i2c_data
{
	...
	int irq;
	int clock_stretch; //iic clock stretch config value [=0 disable, =1 enable]
};
...
static void mtk_i2c_adap_init(struct i2c_adapter *adap)
{
	...
	if(data->clock_stretch == 0)
		mtk_write_fld(data->base+regs->ctl0,I2C_CTL0_SCL_STRETCH, 0x0);
	else
		mtk_write_fld(data->base+regs->ctl0,I2C_CTL0_SCL_STRETCH, 0x1);
...
static int mtk_i2c_wait_hw(struct i2c_adapter *adap)
{
	...
	if(!time_left)
	{
		//pr_info("current thread info tid=%d name=%s",current.tgid, current.comm);
...
static int mtk_i2c_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32_array(pdev->dev.of_node, "stretch-en",
			&stretch,1);
	if(ret < 0){
		dev_warn(&pdev->dev, "read stretch-en property error,erro=%d\n", ret);
		stretch = 0;
	}else{
		pr_info("chan_nr=%d stretch config value=%d \n", chan_nr, stretch);
	}
