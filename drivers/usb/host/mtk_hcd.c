...
{    
    ...
    //  do not handle zero length data except CTS null data test.
    if (!is_null_packet&&(dwCount == 0) )
    {
        return ;
    }
...
static uint8_t MGC_TxPacket(MGC_LinuxCd *pThis, MGC_LinuxLocalEnd *pEnd)
{
    ...
    else
    {
	   ...
	   else if ((pUrb->transfer_flags &USB_ZERO_PACKET) && (pEnd->dwOffset ==0))
	   {	   
	  	  pUrb->transfer_flags &= ~USB_ZERO_PACKET; 
		   bDone = FALSE;
		   bZLP = TRUE;
		   is_null_packet = TRUE;
		   INFO("[USB]Tx Send NULL Data for CTS TEST !\n");
	   }
            else
            {
                bDone = TRUE;
...
static int MUC_hcd_probe(struct platform_device *pdev)
{
	...
	*/
    ...
    retval = usb_add_hcd(hcd, virq[id], IRQF_SHARED);
    if (retval != 0)
    {
        return USB_ST_NOMEMORY;
    }
...
static int __init MUC_init(void)
{
    ...
    for(i = 0; i < MUC_NUM_PLATFORM_DEV; i++)
    {
        virq[i] = irq_of_parse_and_map(np0, i);
        if (virq[i] == 0) {
        INFO("USB %d irq map failed\n",i);
        }
    }
#else
