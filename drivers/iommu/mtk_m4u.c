...
#endif
static DEFINE_RATELIMIT_STATE(_int_log_rs, DEFAULT_RATELIMIT_INTERVAL, DEFAULT_RATELIMIT_BURST);
...
#define pr_intr(fmt,...) if(print)printk(KERN_ERR pr_fmt(fmt), ##__VA_ARGS__)
    ...
    if(tag_valid &(1UL<<(set_num%32)))//valid
    {
        ...
        for(offset=0;offset<L2TLB_DESC_NUM;offset++)
        {
            ...
            if(!offset)
            {
                if(!is_l1)//l2
                {
                    va=((tag &L2TLB_TAG_VIRTUAL_ADDR_MSK)<<L2TLB_TAG_VIRTUAL_ADDR_SHIFT) +(SZ_4K*L2TLB_DESC_NUM*set_num);
                }
                else//l1
                {
                    va=((tag &L2TLB_TAG_VIRTUAL_ADDR_MSK)<<L2TLB_TAG_VIRTUAL_ADDR_SHIFT)/(SZ_1M*L2TLB_DESC_NUM);// +(0x100000*L2TLB_DESC_NUM*set_num);
                    va *=(SZ_1M*L2TLB_DESC_NUM);
                }
                if(!m)
                {
...
static irqreturn_t mtk_m4u_interrupt(int irq, void *dev_id)
{
    ...
    if(__ratelimit(&_int_log_rs))
    {
        print=1;
    }
    pr_intr("irq%d:%s interrupt in domain %p\n",irq,dev_name(data->dev),&(data->m4u_dom->domain));
    
    
    if(u4status&F_L2_TABLE_WALK_FAULT_INT_FAULT)
    {
        pr_intr("L2 table walk fault:");
    ...
    if(!is_mm2)
    {
        ...
        if(u4status & F_INT_MAU0_ASSERT_EN)
        {
            pr_intr("MMU1_MAU0 Interrupt\n");
            _mtk_m4u_dump_mau_info(data,1,0);
        }
        if(u4status & F_INT_MAU1_ASSERT_EN)
        {
            pr_intr("MMU1_MAU1 Interrupt\n");
            _mtk_m4u_dump_mau_info(data,1,1);
        }
        if(u4status & F_INT_MAU2_ASSERT_EN)
        {
            pr_intr("MMU1_MAU2 Interrupt\n");
            _mtk_m4u_dump_mau_info(data,1,2);
        }
        if(u4status & F_INT_MAU2_ASSERT_EN)
        {
            pr_intr("MMU1_MAU3 Interrupt\n");
...
dma_addr_t mtk_m4u_switch_iova_domain(struct device * old_dev,struct device * new_dev,dma_addr_t iova,unsigned long size,enum dma_data_direction dma_dir)
{
    ...
    new_iova=sg_dma_address(sgt.sgl);
    sg_free_table(&sgt);
    kvfree(pages);
    return new_iova;
