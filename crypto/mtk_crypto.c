...
static BOOL GCPU_Init(UINT32 u4Mode)
{
    ...
    if(!_fgInit)
    {
        INT32 virt_gcpu_irq, virt_mmu_irq;
#ifdef CONFIG_OF
        struct device_node *gcpu_dev = NULL;
        struct device_node *mmu_dev = NULL;
#endif
...
#ifdef CONFIG_OF
        gcpu_dev = of_find_compatible_node(NULL, NULL, "Mediatek, GCPU");
        if (gcpu_dev) {
            virt_gcpu_irq = irq_of_parse_and_map(gcpu_dev, 0);
        } else {
            pr_err("%s: Could not find GCPU node\n", __func__);
            virt_gcpu_irq = VECTOR_AES;
        }

        mmu_dev = of_find_compatible_node(NULL, NULL, "mediatek, mt5891-iommu");
        if (mmu_dev) {
            virt_mmu_irq = irq_of_parse_and_map(mmu_dev, 1);
        } else {
            pr_err("%s: Could not find IOMMU node\n", __func__);
            virt_mmu_irq = VECTOR_MMU_GCPU;
        }
        
        printk(KERN_DEBUG "crypto:virt_gcpu_irq: %d\n", virt_gcpu_irq);
        printk(KERN_DEBUG "crypto:virt_mmu_irq: %d\n", virt_mmu_irq);
#else
        virt_gcpu_irq = VECTOR_AES;
        virt_mmu_irq = VECTOR_MMU_GCPU;
#endif
...
#ifdef CONFIG_OF
        if (gcpu_dev) {
            pGcpuIoMap = of_iomap(gcpu_dev, 0);
        } else {
            pGcpuIoMap = ioremap(AES_PHYS, 0x1000);
        }

        if (mmu_dev) {
            pMmuIoMap = of_iomap(mmu_dev, 0);
        } else {
    	...
    	i4Ret = request_irq(virt_gcpu_irq, _AesIrqHandler, 0, "GCPU AES", NULL);
    	if (i4Ret != 0)
    	{
    	...
    	i4Ret = request_irq(virt_mmu_irq, _AesIommuIrqHandler, 0, "GCPU IOMMU", NULL);
    	if (i4Ret != 0)
    	{
...
//#error linux_core/kernel/linux-3.10/crypto/mtk_crypto.c
//-----------------------------------------------------------------------------
