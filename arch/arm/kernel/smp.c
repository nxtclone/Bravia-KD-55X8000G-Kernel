...
void handle_IPI(int ipinr, struct pt_regs *regs)
{
	...
	default:
		...
		if ( ipinr >= IPI_CUSTOM_FIRST && ipinr <= IPI_CUSTOM_LAST){
			if(NULL==ipi_custom_irq_domain)
				pr_crit("CPU%u: TRUSTY DUMMY IPI message 0x%x\n", cpu, ipinr);
			else
...
static void handle_custom_ipi_irq(struct irq_desc *desc)
{
	if (!desc->action) {
		pr_crit("CPU%u: Unknown IPI message 0x%x, no custom handler\n",
			smp_processor_id(), irq_desc->irq_data.irq);
		return;
	}
...
static int __init smp_custom_ipi_init(void)
{
   ...
   if (np) {
       /*
        * Register hwirq that Custom IPI involed.
        * It's between IPI_CUSTOM_FIRST and IPI_CUSTOM_LAST.
        */
