...
static void mt53xx_print_cpuinfo(int cpu)
{
#if defined(CONFIG_ARCH_MT5882)
    pr_err("cpu %d:\n",cpu);
    __raw_writel(0x8+cpu,pMcuIoMap+0x80);
    pr_err("\tpc:%x\n",__raw_readl(pMcuIoMap+0x84));
    __raw_writel(0xc+cpu,pMcuIoMap+0x80);
    pr_err("\tfp:%x\n",__raw_readl(pMcuIoMap+0x84));
#elif defined(CONFIG_ARCH_MT5886) || defined(CONFIG_ARCH_MT5863) 
    pr_err("cpu %d:\n",cpu);
    __raw_writel(0x8+(cpu*2),pMcuIoMap+0x404);
    pr_err("\tpc:%x\n",__raw_readl(pMcuIoMap+0x408));
    __raw_writel(0x10+(cpu*0x10),pMcuIoMap+0x404);
    pr_err("\tfp:%x\n",__raw_readl(pMcuIoMap+0x408));
#else
    UNUSED(cpu);
#endif
...
void mt53xx_secondary_init(unsigned int cpu)
{
...
int mt53xx_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
...
void platform_smp_prepare_cpus_wakeup(void)
{
...
void platform_smp_prepare_cpus_boot(void)
{
	__raw_writel(virt_to_phys(mt53xx_secondary_startup),
		 pBimIoMap+REG_CORE1_UP_ADDR);
	__raw_writel(0xffffb007,
