...
static INT32 _Native_CKGEN_GPIO_Init()
{
	...
	if (!np_ckgen) {
		panic("%s: ckgen node not found\n", __func__);
	}
	tmp_virq = irq_of_parse_and_map(np_ckgen, 0);
#else
#ifdef CC_FIQ_SUPPORT
	tmp_virq = VECTOR_EXT_INT0;
#else
	tmp_virq = VECTOR_EXT1;
#endif
#endif

	i4Ret = request_irq(tmp_virq, _Native_GPIO_DedicatedIsr, IRQF_SHARED, "GPIO", _Native_GPIO_DedicatedIsr);
...
static INT32 _Native_CKGEN_GPIO_Stop(void)
{
    ...
    free_irq(tmp_virq,_Native_GPIO_DedicatedIsr);
