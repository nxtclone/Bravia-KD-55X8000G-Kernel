...
#endif
unsigned long  mt53xx_fb_start = 0 ;
unsigned long  mt53xx_fb_size = 0 ;
static int __init fb_start_set(char *str)
{
    mt53xx_fb_start = simple_strtoul(str, NULL, 16);
    printk(" mt53xx_fb_start =0x%lx\n", mt53xx_fb_start);
    return 1;
}
static int __init fb_size_set(char *str)
{
    mt53xx_fb_size = simple_strtoul(str, NULL, 16);
    printk(" mt53xx_fb_size =0x%lx\n", mt53xx_fb_size);
    return 1;
}
__setup("fb_start=", fb_start_set);
__setup("fb_size=", fb_size_set);
...
void bim_irq_ack(struct irq_data *data)
{
    ...
    if(irq == VECTOR_GDMA || irq == VECTOR_IMGRZ || irq == VECTOR_IMGRZ3)
    {
        return;
    }
...
unsigned int mt53xx_get_ic_version()
{
    ...
    || defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5861) \
    || defined(CONFIG_ARCH_MT5882) || defined(CONFIG_ARCH_MT5865) \
    || defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5893) \
    || defined(CONFIG_ARCH_MT5886) || defined(CONFIG_ARCH_MT5863) \
    || defined(CONFIG_ARCH_MT5866) || defined(CONFIG_ARCH_MT5895)
...
#endif
#ifdef CONFIG_OPM
#if defined(CONFIG_ARCH_MT5886) || defined(CONFIG_ARCH_MT5893) || defined(CONFIG_ARCH_MT5895)
...
#elif defined(CONFIG_ARCH_MT5863) || defined(CONFIG_ARCH_MT5866)
...
#endif
#endif
#if defined(CONFIG_ARCH_MT5886) || defined(CONFIG_ARCH_MT5863) || defined(CONFIG_ARCH_MT5866)
...
phys_addr_t loader_logo_base = 0;
phys_addr_t loader_logo_size = 0;
EXPORT_SYMBOL(loader_logo_base);
EXPORT_SYMBOL(loader_logo_size);
static int __init early_loader_logo(char *p)
{
	pr_debug("%s(%s)\n", __func__, p);
	if(!p)
		return 0;
	loader_logo_size = memparse(p, &p);
	if (*p != '@')
		return 0;
	loader_logo_base = memparse(p + 1, &p);

	return 0;
}
early_param("loaderlogo", early_loader_logo);
...
#endif
    if((loader_logo_base)&&(loader_logo_size)) // reserve memory for long time logo, will be freed after logo is not needed.
    {
        memblock_reserve(loader_logo_base, loader_logo_size);
        printk("logobase= 0x%x, logosize = 0x%x \n", loader_logo_base, loader_logo_size);
    }
}
...
extern char _stext[], __init_begin[];
...
#endif
{
    ...
    if(cmd && !memcmp(cmd, "recovery", 8))
    {
       ...
       if(u4Val == -1)
		...
		#if defined(CONFIG_MACH_MT5890) || defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5893) \
            || defined(CONFIG_ARCH_MT5886) || defined(CONFIG_ARCH_MT5863) || defined(CONFIG_ARCH_MT5866) || defined(CONFIG_ARCH_MT5895)
...
struct platform_device request_hdmi_device ={
	.name = "request_hdmi",
	.id = -1,
} ;

EXPORT_SYMBOL(request_hdmi_device);
...
static void  __init mt53xx_init_machine(void)
{
   ...
   struct proc_dir_entry * proc_file __attribute__((unused)) = 0;
    ...
    #endif
	platform_device_register(&request_hdmi_device);
...
static void irq_polarity_init(void)
{
/*
 * This code cp from soc/mediatek-tv/soc-mt53xx.c
 * MT5891 work well w/o this. Only one possibility we need this
 * is it's relative to polarity.
 *
...
unsigned int dtv_ko_to_virq[NR_IRQS];
unsigned int dtv_ko_to_hwirq[NR_IRQS];
static int __init mt53xx_soc_init(void)
{
        struct device_node *np_dtv_ko;
        int i;
        unsigned int tmp;

        printk("mt53xx_soc_init...\n");

        np_dtv_ko = of_find_compatible_node(NULL, NULL, "Mediatek, DTV-KO");
        for (i=0; i<NUM_IRQ_VECTOR; i++) {
                dtv_ko_to_virq[i] = tmp = irq_of_parse_and_map(np_dtv_ko, i);
                if (tmp >= NR_IRQS) {
                        printk("ERROR! Please enlarge dtv_ko_to_hwirq\n");
                }
                dtv_ko_to_hwirq[tmp] = i;
        }
...
EXPORT_SYMBOL(dtv_ko_to_virq);
EXPORT_SYMBOL(dtv_ko_to_hwirq);
...
#if defined(CONFIG_ARCH_MT5399) || defined(CONFIG_ARCH_MT5398) || defined(CONFIG_ARCH_MT5890) \
    || defined(CONFIG_ARCH_MT5861) || defined(CONFIG_ARCH_MT5882) || defined(CONFIG_ARCH_MT5865) \
    || defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5893) || defined(CONFIG_ARCH_MT5886) \
    ||  defined(CONFIG_ARCH_MT5863) || defined(CONFIG_ARCH_MT5866) || defined(CONFIG_ARCH_MT5895)
...
#if defined(CONFIG_ARCH_MT5399) || defined(CONFIG_ARCH_MT5398) || defined(CONFIG_ARCH_MT5880) \
    || defined(CONFIG_ARCH_MT5890) || defined(CONFIG_ARCH_MT5861) || defined(CONFIG_ARCH_MT5882) \
    || defined(CONFIG_ARCH_MT5865) || defined(CONFIG_ARCH_MT5891) || defined(CONFIG_ARCH_MT5893)  \
    || defined(CONFIG_ARCH_MT5886) || defined(CONFIG_ARCH_MT5863) || defined(CONFIG_ARCH_MT5866) \
    || defined(CONFIG_ARCH_MT5895)
...
#elif defined(CONFIG_MACH_MT5895)
DT_MACHINE_START(MT53XX, "MT5895")
...
unsigned int isSWregNonCache = 0;
static int __init setSWregNonCacheMemory(char *str)
{
    printk(KERN_EMERG "SW register allocate non-cache memory\n");
    isSWregNonCache = 1;
    return 1;
}
__setup("NCSToolConnect", setSWregNonCacheMemory);

unsigned int isSWregNoncacheMemory(void)
{
    return isSWregNonCache;
}
EXPORT_SYMBOL(isSWregNoncacheMemory);
...
void local_touch_nmi_watchdog(void)
{
    ...
    __raw_writel(0x742FFFFF, pPdwncIoMap + REG_RW_WATCHDOG_TMR);
    /* reset S/W watchdog timer (timer1) */
    __bim_writel(0x05B8D800, REG_RW_TIMER1_LOW);
    __bim_writel(0x0, REG_RW_TIMER1_HLMT);
}
...
#elif defined(CONFIG_MACH_MT5895)
MACHINE_START(MT5895, "MT5895")
#elif defined(CONFIG_MACH_MT5886)
MACHINE_START(MT5886, "MT5886")
#elif defined(CONFIG_MACH_MT5863)
MACHINE_START(MT5863, "MT5863")
#elif defined(CONFIG_MACH_MT5866)
MACHINE_START(MT5863, "MT5866")
#else
