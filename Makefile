...
objtree		:= .
objtree		:= $(if $(KBUILD_OUTPUT),$(KBUILD_OUTPUT),.)
src		:= $(srctree)
obj		:= $(objtree)
...
ifeq "$(ODB)" "true"
AS      = $(ODB_CMD) $(CROSS_COMPILE)as
CC      = $(ODB_CMD) $(CROSS_COMPILE)gcc
else
AS      = $(CROSS_COMPILE)as
...
USERINCLUDE    += \
		-I$(srctree)/kernel \
		-I$(srctree)/drivers/mmc_common/host \
...
KBUILD_CFLAGS_KERNEL := -DMTK_DEFINE_CODE_CLEAR
...
MTK_EXTMOD_M     := $(word 2, $(subst /$(LINUX_ROOT_NAME)/,/$(LINUX_ROOT_NAME)/ , $(KBUILD_EXTMOD)))
ifdef DTV_DRIVER_KO
MTK_EXTMOD_OBJ   := $(OBJ_ROOT)
else
...
# Final link of vmlinux with optional arch pass after final link
    cmd_link-vmlinux =                                                 \
	CC="$(filter-out odbc ccache, $(CC))" $(CONFIG_SHELL) $< $(LD) $(LDFLAGS) $(LDFLAGS_vmlinux) ;       \
...
.PHONY: $(PHONY)
#Add the flag to stop latent entropy plugin from adding random values into the KO file
ifdef CONFIG_GCC_PLUGIN_LATENT_ENTROPY
  CFLAGS_MODULE += -fplugin-arg-latent_entropy_plugin-disable
endif
